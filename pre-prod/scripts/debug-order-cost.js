...
// --- CostCalculator Class (Copied for standalone execution) ---
class CostCalculator {
  ...
  async calculateProductCosts(sku) {
    try {
      ...
      console.log('Shipping Group:', skuMapping?.merchant_shipping_group || 'Off Amazon (Default)');
      ...
      console.log(`TOTAL ESTIMATED COST: £${(materialTotalCost + shippingCost).toFixed(2)}`);
      console.log('----------------------\n');
...
async function debugOrder(orderId) {
  ...
  console.log(`Order Found. Total: ${order.order_total} ${order.currency_code}`);
  ...
  for (const item of order.amazon_order_items) {
    if (item.seller_sku) {
      await calculator.calculateProductCosts(item.seller_sku);
    } else {
...
const orderId = '203-0095555-2155559';
debugOrder(orderId);
