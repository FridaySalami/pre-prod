...
async function clearBuyBoxData() {
  console.log('Clearing all data from buybox_data table...');

  try {
    // Delete all records from the buybox_data table
    ...
    console.log('Successfully cleared all data from buybox_data table');
    return data;
  } catch (error) {
    console.error('Error clearing buybox_data:', error);
    throw error;
  }
}
...
------------------------------------
1. All data in buybox_data table has been cleared
2. A new Buy Box check job has started with ID: ${jobId}
3. Processed ${testAsins.length} ASINs as a test run
4. Job is marked as PARTIAL - ${asins.length - testAsins.length} ASINs remain to be processed

Next steps:
1. Run the monitor-rate-limiting.js script to check job progress
2. Examine the new buybox_data entries to ensure costing and data fields are properly populated
3. If entries look good, resume processing remaining ASINs with restart-buybox-job.js
`);
...
module.exports = { clearBuyBoxData, createNewBuyBoxJob };
