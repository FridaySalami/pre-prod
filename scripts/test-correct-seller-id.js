// Test with the correct seller ID
import { createClient } from '@supabase/supabase-js';
import dotenv from 'dotenv';

dotenv.config();

// Standalone Amazon Feeds API using correct seller ID
class AmazonFeedsAPI {
  constructor() {
    ...
    console.log('✅ AmazonFeedsAPI initialized for testing with correct seller ID');
  }
  ...
  async updatePrice(asin, newPrice, currentPrice = null, sku = null, productType = 'CONDIMENT') {
    ...
    try {
      ...
      console.log(`   🔸 Using Seller ID: A2D8NG39VURSL3`);
      ...
      console.log('📤 Step 2: Uploading JSON pricing data with correct seller ID...');
  ...
  async uploadPricingData(uploadUrl, asin, sku, price, productType) {
    // Create pricing feed data in JSON format with correct seller ID
    const jsonData = {
      "header": {
    ...
    console.log('📋 Pricing feed data (JSON format with correct seller ID):');
    ...
    console.log('✅ JSON pricing data with correct seller ID uploaded to S3');
  }
...
// Test the implementation with correct seller ID
async function testWithCorrectSellerID() {
  console.log('🧪 Testing JSON_LISTINGS_FEED with correct seller ID (A2D8NG39VURSL3)...\n');
  ...
  try {
    ...
    console.log('🎯 Using correct seller ID: A2D8NG39VURSL3');
