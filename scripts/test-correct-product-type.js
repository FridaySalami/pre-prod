// Test with the correct product type directly
import { createClient } from '@supabase/supabase-js';
import dotenv from 'dotenv';

dotenv.config();

// Standalone Amazon Feeds API using modern JSON format with correct product type
class AmazonFeedsAPI {
  constructor() {
    ...
    console.log('✅ AmazonFeedsAPI initialized for testing with correct product type');
  }

  async updatePrice(asin, newPrice, currentPrice = null, sku = null, productType = 'CONDIMENT') {
    ...
    try {
      ...
      console.log('📤 Step 2: Uploading JSON pricing data with correct product type...');
  ...
  async uploadPricingData(uploadUrl, asin, sku, price, productType) {
    // Create pricing feed data in JSON format with proper header structure and correct product type
    const jsonData = {
      "header": {
        "sellerId": this.config.clientId,
        "version": "2.0"
      },
      "messages": [
        {
          ...
          "productType": productType, // Use the correct product type
          "attributes": {
            "purchasable_offer": [
              {
    ...
    console.log('📋 Pricing feed data (JSON format with correct product type):');
    ...
    console.log('✅ JSON pricing data with correct product type uploaded to S3');
  }
...
// Test the implementation with correct product type
async function testWithCorrectProductType() {
  console.log('🧪 Testing JSON_LISTINGS_FEED with correct product type (CONDIMENT)...\n');

  const api = new AmazonFeedsAPI();

  try {
    const result = await api.updatePrice('B08BPBWV1C', 42.15, 42.16, 'COL01A', 'CONDIMENT');

    console.log('\n🎉 Final Result:');
    console.log(JSON.stringify(result, null, 2));

    console.log(`\n✅ SUCCESS! JSON Feed ID: ${result.feedId}`);
    console.log('⏰ Check feed status in 2-3 minutes');
    console.log('🎯 Using correct product type: CONDIMENT');
