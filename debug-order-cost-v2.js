...
class CostCalculator {
  ...
  async calculateProductCosts(sku, price = 0, options = {}) {
    try {
      ...
      if (options.isPrime) {
        console.log('Order is Prime: Forcing "Nationwide Prime" shipping.');
      ...
      const amazonFeeRate = this.getAmazonFeeRate(price);
      ...
      console.log(`Base Cost (Linnworks): £${baseCost.toFixed(2)}`);
      console.log(`VAT Amount: £${vatAmount.toFixed(2)} (Rate: ${vatCode}%)`);
      console.log(`Box Cost: £${boxCost.toFixed(2)}`);
      console.log(`Material Cost: £${materialCost.toFixed(2)}`);
      console.log(`Fragile Charge: £${fragileCharge.toFixed(2)}`);
      console.log(`Shipping Cost: £${shippingCost.toFixed(2)} (Tier: ${shipping})`);
      console.log(`Amazon Fee: £${amazonFee.toFixed(2)} (Rate: ${(amazonFeeRate * 100).toFixed(1)}% on £${price.toFixed(2)})`);
      console.log(`TOTAL ESTIMATED COST: £${(materialTotalCost + shippingCost + amazonFee).toFixed(2)}`);
      console.log('----------------------\n');
...
async function debugOrder(orderId) {
  ...
  console.log(`Order Found. Total: ${order.order_total} ${order.currency_code}. Prime: ${order.is_prime}`);
  console.log(`Items: ${order.amazon_order_items.length}`);
  ...
  for (const item of order.amazon_order_items) {
    if (item.seller_sku) {
      ...
      await calculator.calculateProductCosts(item.seller_sku, itemPrice, { isPrime: order.is_prime });
    } else {
      console.log('Item has no SKU:', item.title);
    }
  }
}
