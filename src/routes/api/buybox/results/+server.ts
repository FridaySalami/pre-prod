...
/**
 * Get Buy Box job results (proxy to Render service)
 */
export async function GET({ url }) {
  try {
    ...
    const renderUrl = `${RENDER_SERVICE_URL}/api/job-results${queryString ? '?' + queryString : ''}`;
    ...
    console.error('Buy Box results proxy error:', error);
    return json({
      success: false,
