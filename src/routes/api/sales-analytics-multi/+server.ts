...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const month = url.searchParams.get('month') || 'june'; // Allow month selection
    const limit = parseInt(url.searchParams.get('limit') || '50');
    const offset = parseInt(url.searchParams.get('offset') || '0');

    // Determine table name based on month
    ...
    const tableName = tableMap[month.toLowerCase()] || 'sales_june';
    ...
    if (error) {
      ...
      return json({ error: `Failed to fetch sales data from ${tableName}` }, { status: 500 });
    }
    ...
    return json({
      ...
      searchType,
      month: tableName,
      availableMonths: Object.keys(tableMap)
    });
