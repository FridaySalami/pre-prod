<script lang="ts">
  ...
  $: totalTime = processSteps.reduce((sum, step) => sum + (step.time_minutes || 0), 0);
  $: stepsWithIssues = processSteps.filter((step) => step.issues && step.issues !== '').length;
                          ...
                          <div class="issue-item" class:expanded={i < 3}>
                            {#if typeof issue === 'object' && issue !== null}
                              ...
                              <div class="issue-header">
                                {#if issueObj.status}
                                  <span class="issue-status status-{issueObj.status.toLowerCase()}"
...
{#if showDeleteConfirmModal}
  <div class="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="delete-confirm-title">
