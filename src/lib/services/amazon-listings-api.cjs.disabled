...
class AmazonListingsAPI {
   ...
   * @param {string} [config.clientId] - Amazon API client ID
   * @param {string} [config.clientSecret] - Amazon API client secret
   * @param {string} [config.refreshToken] - Amazon API refresh token
   * @param {string} [config.marketplaceId] - Amazon marketplace ID
   * @param {string} [config.awsAccessKeyId] - AWS Access Key ID
   * @param {string} [config.awsSecretAccessKey] - AWS Secret Access Key
  ...
  constructor(config = {}) {
    this.config = {
      ...
      endpoint: config.environment === 'production'
        ...
        : 'https://sandbox.sellingpartnerapi-eu.amazon.com',      // API credentials from environment
   ...
   * @param {string|null} [sku=null] - Product SKU (optional)
   * @returns {Promise<Object>} API response
   */
  async updatePrice(asin, newPrice, sku = null) {
    try {
      const token = await this.getAccessToken();
...
module.exports = AmazonListingsAPI;
