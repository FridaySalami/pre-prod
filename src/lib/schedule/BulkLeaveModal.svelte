	...
	async function calculateWorkingDays(start: string, end: string, employeeId: string) {
		...
		try {
			...
			return allDates.filter((date) => {
				...
				return patterns.some((p) => p.day_of_week === patternDay && p.is_working) || false;
			});
		} catch (err) {
...
<div class="modal-backdrop" on:click|self={() => dispatch('close')} role="presentation">
	<div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
		<div class="modal-header">
			<h2 id="modal-title">Add Leave</h2>
			<button class="close-button" on:click={() => dispatch('close')} aria-label="Close modal">
				<svg
				...
				<div class="form-row">
					<div class="form-column">
						...
						<input
							id="startDate"
							type="date"
							bind:value={bulkLeave.startDate}
							required
							disabled={loading}
						/>
					</div>
					...
					<div class="date-preview">
						<div class="preview-header">
							...
							<span class="preview-count"
								>{$previewDates.length} working day{$previewDates.length !== 1 ? 's' : ''}</span
							>
						</div>
						<div class="date-list">
