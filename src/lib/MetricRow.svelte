<script context="module" lang="ts">
  ...
  <script lang="ts">
    ...
    // "values" here are the computed daily values for this metric.
...
export let previousTotal: string;
    ...
    export let openNotesModal: (metricIndex: number, dayIndex: number) => void;
    ...
    <td class="metric-name">
      ...
      {#if metricField === null}
        <div class="calc-info-container">
          ...
          <div class="calc-tooltip">Calculation details here</div>
        </div>
      {/if}
    </td>
          ...
          <span
            ...
            on:keydown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                openNotesModal(metricIndex, dayIndex);
              }
            }}
          >
            {value}
